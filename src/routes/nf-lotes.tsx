...
type LotLine={id:string;company:Company;name:string;code:string;lot:string;expiry:string;quantity:number};
...
function NfLotes(){
 ...
 <section className="panel nf-builder"><div className="nf-builder-head"><div><div className="section-label">NOVA NOTA FISCAL</div><h3>Montar NF</h3><p>O mesmo produto pode aparecer várias vezes quando o lote for diferente.</p></div><label>Número da NF<input value={nf} onChange={e=>setNf(e.target.value)} placeholder="Ex.: 000123"/></label></div>
 <div className="nf-line-form"><label>Empresa<select value={line.company} onChange={e=>setLine({...line,company:e.target.value as Company})}><option>Pharma</option><option>Aspen</option><option>Viatris</option></select></label><label>Produto<input value={line.name} onChange={e=>setLine({...line,name:e.target.value})} placeholder="Produto 500mg"/></label><label>Código<input value={line.code} onChange={e=>setLine({...line,code:e.target.value})} placeholder="7891234567890"/></label><label>Lote<input value={line.lot} onChange={e=>setLine({...line,lot:e.target.value})} placeholder="AB155210"/></label><label>Validade<input type="date" value={line.expiry} onChange={e=>setLine({...line,expiry:e.target.value})}/></label><label>Quantidade<input type="number" min="1" step="1" value={line.quantity} onChange={e=>setLine({...line,quantity:e.target.value})}/></label><button className="primary-btn" onClick={addLine}><Plus size={16}/> Adicionar lote</button></div>
 <div className="nf-lines">{lines.map((l,i)=><div className="nf-line" key={l.id}><div className="nf-index">{i+1}</div><div><strong>{l.name}</strong><span>{l.company} · Código {l.code}</span></div><code>{l.lot}</code><span>Val. {formatDate(l.expiry)}</span><b>Qtd. {l.quantity}</b><button onClick={()=>setLines(x=>x.filter(y=>y.id!==l.id))} aria-label="Remover lote"><Trash2 size={15}/></button></div>)}{!lines.length&&<div className="empty-inline">Nenhum lote adicionado. Para dois lotes do mesmo produto, adicione o primeiro lote e depois o segundo.</div>}</div>
